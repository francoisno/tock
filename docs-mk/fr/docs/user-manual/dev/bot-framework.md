...
Le _Bot Framework_ Tock permet de développer un bot en mode intégré, en utilisant un 
...
Attention : dans ce mode de développement, contrairement au mode _Bot API_, il est nécessaire que le module bot 
 dispose d'une connexion à la base de donnée (MongoDB) de la plateforme Tock utilisée.

> Pour appréhender complètement ce qui va suivre, il est recommandé de maîtriser les bases du langage [Kotlin](https://kotlinlang.org/).
...
à une ou plusieurs intentions dites "secondaires" (voir [Concepts](../concepts.md)).
...
Pour en savoir plus sur les différents connecteurs, voir [cette page](../connectors.md).
...
Le nouveau connecteur doit alors être disponible dans l'interface [_Bot Configurations_](../studio/configuration.md) de _Tock Studio_.
